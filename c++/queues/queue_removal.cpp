...
// Complexity: Time= O(n + x ^ 2), Space = O(n + x)
...
vector <int> findPositions(vector <int> arr, int x) {
  ...
  // Add all elements to queue => O(n)
  for (int i = 0; i < (int) arr.size(); i++) {
    q.push({arr[i], i + 1 /* original 1-based index */});
  }
  
  // Total operation => O(x * x/2)
  // Perform x iterations => O(x)
  for (int i = 0; i < x; i++) {
    int count = 0;
    // Keep track of maximum for every iteration
    qEntry qMax{INT_MIN, -1};
    // Store popped up element
    vector<qEntry> poppedElements;
    
    // Pop up to x elements => O(x/2) - average
    while (!q.empty() && count < x) {
      qEntry qCurrent = q.front();
      if (qCurrent.value > qMax.value) {
        qMax = qCurrent;
      }
      q.pop();
      count++;
      // Add all processed element to poppedElements vector
      poppedElements.push_back(qCurrent);
    }
    // Add original index of maximum element popped to output list
    if (qMax.index != -1) {
      result.push_back(qMax.index);
    }
    // Pop adjusted elemets back, except the maximum element removed => O(x/2) - average
    for (int j = 0; j < (int) poppedElements.size(); j++) {
      if (qMax.index != -1 && poppedElements[j].index != qMax.index) {
        int adjustedVal = poppedElements[j].value;
        adjustedVal = (adjustedVal > 0 ? adjustedVal - 1 : adjustedVal);
        q.push({adjustedVal, poppedElements[j].index});
      }
    }
  }
  return result;
